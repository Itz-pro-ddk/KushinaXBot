...
from kushina.language import get_lang_file
...
class TelegramBot(MixinBase):
    ...
    async def init_client(self: "kushina") -> None:
        ...
        file = AsyncPath("kushina/kushina.session")
    ...
    async def start(self: "Kushina") -> None:
        if self.__running:
    ...
    async def idle(self: "Kushina") -> None:
        if self.__running:
            raise RuntimeError("This bot instance is already running")
    ...
    @property
    def events_activated(self: "Kushina") -> int:
        return len(self._plugin_event_handlers)

    def redact_message(self: "Kushina", text: str) -> str:
